...
TEST_CASE("Test edge_length() function for 'double' type.",
  ...
  Vec3D::set_eps(1E-7);
  const Vec3D u = {1.0, 2.0, 3.0};
  const Vec3D v = {4.0, 5.0, 6.0};

  constexpr double eps = 1E-14;
  const double expected = sqrt(27.0);
  const double actual_d1 = edge_length(u, v);
  const double actual_d2 = edge_length(v, u);
...
TEST_CASE("Test edge_length() function for 'multiprecision' type.",
  ...
  const mpreal expected = mpfr::sqrt(27.0);
  const mpreal actual_d1 = edge_length(u, v);
  const mpreal actual_d2 = edge_length(v, u);
...
TEST_CASE("Test edge_center() function for 'double' type.",
  ...
  Vec3D::set_eps(1E-7);
  const Vec3D r1 = {1.0, 2.0, 3.0};
  const Vec3D r2 = {4.0, 5.0, 6.0};

  const Vec3D expected = {5.0 / 2.0, 7.0 / 2.0, 9.0 / 2.0};

  const Vec3D actual = edge_center(r1, r2);
...
TEST_CASE("Test edge_center() function for 'multiprecision' type.",
  ...
  const Vec3D expected = {mpreal(5.0) / mpreal(2.0),
...
TEST_CASE("Test edge_orientation() function for 'double' type.",
  ...
  const Vec3D expected = {3.0 / sqrt(27.0),
                          3.0 / sqrt(27.0),
                          3.0 / sqrt(27.0)};

  const Vec3D actual = edge_orientation(r1, r2);
...
TEST_CASE("Test edge_orientation() function for 'multiprecision' type.",
  ...
  const Vec3D expected = {3.0 / mpfr::sqrt(27.0),
                          3.0 / mpfr::sqrt(27.0),
                          3.0 / mpfr::sqrt(27.0)};
...
TEST_CASE("Test triangle_normal() function for 'double' type.",
  ...
  Vec3D::set_eps(1E-7);
  const Vec3D r1 = {1.0, 0.0, 0.0};
  const Vec3D r2 = {0.0, 1.0, 0.0};
  const Vec3D r3 = {0.0, 0.0, 1.0};

  const Vec3D expected = {1.0 / sqrt(3),
                          1.0 / sqrt(3.0),
                          1.0 / sqrt(3.0)};

  const Vec3D actual = triangle_normal(r1, r2, r3);
...
TEST_CASE("Test triangle_normal() function for 'multiprecision' type.",
  ...
  const mpreal eps = 1E-40;
  const Vec3D expected = {1.0 / mpfr::sqrt(3.0),
                          1.0 / mpfr::sqrt(3.0),
                          1.0 / mpfr::sqrt(3.0)};
...
TEST_CASE("Test triangle_center() function for 'double' type.",
  ...
  const Vec3D expected = {1.0 / 3.0,
                          1.0 / 3.0,
                          1.0 / 3.0};

  const Vec3D actual = triangle_center(r1, r2, r3);
...
TEST_CASE("Test triangle_center() function for 'multiprecision' type.",
  ...
  const mpreal eps = 1E-40;
  const Vec3D expected = {mpreal(1.0) / mpreal(3.0),
...
TEST_CASE("rotation3x3_mpreal",
  ...
  Vector3D v = {mpreal{1.0}, mpreal{2.0}, mpreal{3.0}};
...
TEST_CASE("rotation4x4_mpreal_4d",
  ...
  Vector4D v = {mpreal{1}, mpreal{2}, mpreal{3}, mpreal{1}};
