...
#ifndef LIBFABBRI_INCLUDE_WRITER_XDMF_HPP_
#define LIBFABBRI_INCLUDE_WRITER_XDMF_HPP_
...
class XDMFWriterException : std::exception {
 ...
 public:
   ...
   */
  explicit
  XDMFWriterException(std::string
  message) :
...
class XDMFFileWriter {
 ...
 public:
  ...
  write(const std::string &file_name,
    ...
    std::stringstream ss_nodes_per_element;
    ss_nodes_per_element << DIM + 1;
    ...
    if (DIM == 2) {
      ...
      throw XDMFWriterException("Unsupported XDMF writer dimensions");
    }

    std::string nodes_per_element = ss_nodes_per_element.str();
    ...
    std::stringstream ss_no_of_elems_xDIM;
    ...
    ss_no_of_elems << model.mesh().cil().size();
    ss_no_of_verts_x3 << model.mesh().vcl().size() << " 3";
    ss_no_of_elems_x1 << model.mesh().cil().size() << " 1";
    ss_no_of_elems_xDIM << model.mesh().cil().size() << " " << nodes_per_element;
    ...
    std::string dim_no_of_elems_xDIM = ss_no_of_elems_xDIM.str();
    ...
    for (const auto &field : model.field_list().fields()) {
      ...
      topology->append_attribute(doc.allocate_attribute("TopologyType", topology_type.c_str()));
      topology->append_attribute(doc.allocate_attribute("NumberOfElements", ss_no_of_elems.str().c_str()));
      ...
      topo_data_item->append_attribute(doc.allocate_attribute("Dimensions", dim_no_of_elems_xDIM.c_str()));
...
#endif //LIBFABBRI_INCLUDE_WRITER_XDMF_HPP_