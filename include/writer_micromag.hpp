...
#ifndef LIBFABBRI_INCLUDE_WRITER_MICROMAG_HPP_
#define LIBFABBRI_INCLUDE_WRITER_MICROMAG_HPP_
...
class MicromagFileWriter {
 ...
 public:
   ...
   */
  static void
  write(const std::string &file_name, const Model<DIM> &model) {
 ...
 private:
   ...
   */
  static void
  write_mesh(H5::H5File &file, const Model<DIM> &model) {
   ...
   */
  static void
  write_vertices(H5::H5File &file, const Model<DIM> &model) {
   ...
   */
  static void
  write_elements(H5::H5File &file, const Model<DIM> &model) {

    // Create an elements dataset in the mesh group.
    hsize_t dim_elements[2];
    dim_elements[0] = model.mesh().cil().size();
    dim_elements[1] = DIM + 1;
    ...
    ds_elements.write(model.mesh().cil().data(), H5::PredType::NATIVE_UINT64);
   ...
   */
  static void
  write_submesh_indices(H5::H5File &file, const Model<DIM> &model) {
  ...
  static void
  write_fields(H5::H5File &file, const Model<DIM> &model) {
  ...
  write_field(H5::H5File &file, const Field &field, size_t id) {
    ...
    // Create the attribute for the group.
...
#endif //LIBFABBRI_INCLUDE_WRITER_MICROMAG_HPP_
